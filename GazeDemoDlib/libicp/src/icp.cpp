...
Icp::Icp (double *M,const int32_t M_num,const int32_t dim) :
  dim(dim), max_iter(15), min_delta(1e-3) {
...
void Icp::fit(double *T, const int32_t T_num, Mat::Matrix &R, Mat::Matrix &t, const double indist) {
...
void Icp::fitIterate(double *T, const int32_t T_num, Mat::Matrix &R, Mat::Matrix &t, const std::vector<int32_t> &active) {
  ...
  for (int32_t iter = 0; iter < max_iter; iter++) {
	  double delta = fitStep(T, T_num, R, t, active);
	  //cout << "iter: " << iter << ", residual: " << delta << endl;
	  if (delta < min_delta)
		  break;
  }
}
