...
Mat::Matrix::Matrix(const int32_t m_, const int32_t n_) {
  allocateMemory(m_,n_);
}

Mat::Matrix::Matrix(const int32_t m_, const int32_t n_, const FLOAT* val_) {
  allocateMemory(m_,n_);
  int32_t k=0;
...
Mat::Matrix::Matrix(const Mat::Matrix &M) {
...
Mat::Matrix& Mat::Matrix::operator= (const Mat::Matrix &M) {
  if (this!=&M) {
    if (M.m!=m || M.n!=n) {
      releaseMemory();
      allocateMemory(M.m,M.n);
...
void Mat::Matrix::getData(FLOAT* val_, int32_t i1, int32_t j1, int32_t i2, int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  int32_t k=0;
...
Mat::Matrix Mat::Matrix::getMat(int32_t i1, int32_t j1, int32_t i2, int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
...
void Mat::Matrix::setMat(const Mat::Matrix &M, const int32_t i1, const int32_t j1) {
...
void Mat::Matrix::setVal(FLOAT s, int32_t i1, int32_t j1, int32_t i2, int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
...
void Mat::Matrix::setDiag(FLOAT s, int32_t i1, int32_t i2) {
  if (i2==-1) i2 = min(m-1,n-1);
  for (int32_t i=i1; i<=i2; i++)
    val[i][i] = s;
}

void Mat::Matrix::zero() {
  setVal(0);
}

Mat::Matrix Mat::Matrix::extractCols(vector<int> idx) {
	Mat::Matrix M(m, idx.size());
  for (int32_t j=0; j<M.n; j++)
    if (idx[j]<n)
...
Mat::Matrix Mat::Matrix::eye(const int32_t m) {
	Mat::Matrix M(m, m);
  for (int32_t i=0; i<m; i++)
    M.val[i][i] = 1;
  return M;
...
Mat::Matrix Mat::Matrix::ones(const int32_t m, const int32_t n) {
  Matrix M(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      M.val[i][j] = 1;
...
Mat::Matrix Mat::Matrix::diag(const Mat::Matrix &M) {
  if (M.m>1 && M.n==1) {
	  Mat::Matrix D(M.m, M.m);
    for (int32_t i=0; i<M.m; i++)
      D.val[i][i] = M.val[i][0];
    return D;
  } else if (M.m==1 && M.n>1) {
	  Mat::Matrix D(M.n, M.n);
...
Mat::Matrix Mat::Matrix::reshape(const Mat::Matrix &M, int32_t m_, int32_t n_) {
  if (M.m*M.n != m_*n_) {
...
Mat::Matrix Mat::Matrix::rotMatX(const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  Matrix R(3,3);
  R.val[0][0] = +1;
...
Mat::Matrix Mat::Matrix::rotMatY(const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  Mat::Matrix R(3, 3);
  R.val[0][0] = +c;
...
Mat::Matrix Mat::Matrix::rotMatZ(const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  Mat::Matrix R(3, 3);
  R.val[0][0] = +c;
...
Mat::Matrix Mat::Matrix::operator- (const Mat::Matrix &M) {
	const Mat::Matrix &A = *this;
	const Mat::Matrix &B = M;
  if (A.m!=B.m || A.n!=B.n) {
...
Mat::Matrix Mat::Matrix::operator* (const FLOAT &s) {
	Mat::Matrix C(m, n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      C.val[i][j] = val[i][j]*s;
...
Mat::Matrix Mat::Matrix::operator- () {
	Mat::Matrix C(m, n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      C.val[i][j] = -val[i][j];
...
Mat::Matrix Mat::Matrix::cross(const Matrix &a, const Matrix &b) {
...
Mat::Matrix Mat::Matrix::inv(const Mat::Matrix &M) {
  if (M.m!=M.n) {
    cerr << "ERROR: Trying to invert matrix of size (" << M.m << "x" << M.n << ")" << endl;
    exit(0);
  }
  Mat::Matrix A(M);
  Mat::Matrix B = eye(M.m);
  B.solve(A);
  return B;
}
...
bool Mat::Matrix::solve(const Mat::Matrix &M, FLOAT eps) {
  
  // substitutes
	const Mat::Matrix &A = M;
	Mat::Matrix &B = *this;
...
bool Mat::Matrix::lu(int32_t *idx, FLOAT &d, FLOAT eps) {
...
void Mat::Matrix::svd(Mat::Matrix &U2, Mat::Matrix &W, Mat::Matrix &V) {

	Mat::Matrix U = Mat::Matrix(*this);
  U2 = Mat::Matrix(m, m);
  V = Mat::Matrix(n, n);
...
ostream& operator<< (ostream& out, const Mat::Matrix& M) {
  if (M.m==0 || M.n==0) {
    out << "[empty matrix]";
  } else {
    char buffer[1024];
...
void Mat::Matrix::allocateMemory(const int32_t m_, const int32_t n_) {
  m = abs(m_); n = abs(n_);
  if (m==0 || n==0) {
    val = 0;
    return;
...
FLOAT Mat::Matrix::pythag(FLOAT a, FLOAT b) {
  FLOAT absa,absb;
  absa = fabs(a);
  absb = fabs(b);
  if (absa > absb)
