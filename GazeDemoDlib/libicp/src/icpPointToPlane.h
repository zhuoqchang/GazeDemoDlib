...
public:
  ...
  IcpPointToPlane (double *M,const int32_t M_num,const int32_t dim,const int32_t num_neighbors=5,const double flatness=5.0) : Icp(M,M_num,dim) {
...
private:
	...
	double fitStep(double *T, const int32_t T_num, Mat::Matrix &R, Mat::Matrix &t, const std::vector<int32_t> &active);
	std::vector<int32_t> getInliers(double *T, const int32_t T_num, const Mat::Matrix &R, const Mat::Matrix &t, const double indist);
