...
public:
  ...
  void fit(double *T, const int32_t T_num, Mat::Matrix &R, Mat::Matrix &t, const double indist);
  
private:
  
  // iterative fitting
	void fitIterate(double *T, const int32_t T_num, Mat::Matrix &R, Mat::Matrix &t, const std::vector<int32_t> &active);
  
  // inherited classes need to overwrite these functions
	virtual double               fitStep(double *T, const int32_t T_num, Mat::Matrix &R, Mat::Matrix &t, const std::vector<int32_t> &active) = 0;
	virtual std::vector<int32_t> getInliers(double *T, const int32_t T_num, const Mat::Matrix &R, const Mat::Matrix &t, const double indist) = 0;
