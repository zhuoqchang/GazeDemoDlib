...
static inline Mat::Matrix lssolvesvd(Mat::Matrix &A, Mat::Matrix &b)
{
	int i;
	Mat::Matrix U, S, V;

	A.svd(U, S, V);
	Mat::Matrix Uc = U.getMat(0, 0, A.m - 1, A.n - 1); // compact U
	Mat::Matrix Uctb = ~Uc*b;
	for (i = 0; i<Uctb.m; ++i)
...
double IcpPointToPlane::fitStep(double *T, const int32_t T_num, Mat::Matrix &R, Mat::Matrix &t, const std::vector<int32_t> &active) {

	int i;
	int nact = (int)active.size();

	// init matrix for point correspondences
	Mat::Matrix p_m(nact, dim); // model
	Mat::Matrix p_t(nact, dim); // template
...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,A,b,r00,r01,r10,r11,t0,t1) // schedule (dynamic,2)
		...
		// rotation matrix
		Mat::Matrix R_ = Mat::Matrix::eye(2);
		...
		if (R_.det() < 0){
			Mat::Matrix B = Mat::Matrix::eye(dim);
		...
		R = R_*R;
		t = R_*t + t_;
		return max((R_ - Mat::Matrix::eye(2)).l2norm(), t_.l2norm());
...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,A,b,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2) // schedule (dynamic,2)
		...
		// rotation matrix
		Mat::Matrix R_ = Mat::Matrix::eye(3);
		...
		R = R_*R;
		t = R_*t + t_;
		return max((R_ - Mat::Matrix::eye(3)).l2norm(), t_.l2norm());
	}
...
std::vector<int32_t> IcpPointToPlane::getInliers(double *T, const int32_t T_num, const Mat::Matrix &R, const Mat::Matrix &t, const double indist) {
...
void IcpPointToPlane::computeNormal(const kdtree::KDTreeResultVector &neighbors, double *M_normal, const double flatness) {
	...
	if (dim == 2) {
		...
		Mat::Matrix Q = P - Mat::Matrix::ones(neighbors.size(), 1)*mu;
