...
double IcpPointToPoint::fitStep(double *T, const int32_t T_num, Mat::Matrix &R, Mat::Matrix &t, const std::vector<int32_t> &active) {
  ...
  // init mean
  Mat::Matrix mu_m(1, dim);
  Mat::Matrix mu_t(1, dim);
...
#pragma omp parallel for private(i) default(none) shared(T,active,nact,p_m,p_t,r00,r01,r02,r10,r11,r12,r20,r21,r22,t0,t1,t2) reduction(+:mum0,mum1,mum2, mut0,mut1,mut2) // schedule (dynamic,2)
  ...
  Mat::Matrix q_m = p_m - Mat::Matrix::ones(active.size(), 1)*mu_m;
  Mat::Matrix q_t = p_t - Mat::Matrix::ones(active.size(), 1)*mu_t;

  // compute relative rotation matrix R and translation vector t
  Mat::Matrix H = ~q_t*q_m;
  Mat::Matrix U, W, V;
  H.svd(U,W,V);
  Mat::Matrix R_ = V*~U;
  ...
  if (dim == 2) return max((R_ - Mat::Matrix::eye(2)).l2norm(), t_.l2norm());
  else        return max((R_ - Mat::Matrix::eye(3)).l2norm(), t_.l2norm());
}

std::vector<int32_t> IcpPointToPoint::getInliers(double *T, const int32_t T_num, const Mat::Matrix &R, const Mat::Matrix &t, const double indist) {
